...
MY_PROBE_FILE = "/etc/bitscope/bitscope.prb" # default probe file if unspecified 
MY_MODE = BL_MODE_FAST # preferred capture mode
MY_RATE = 10 # default sample rate we'll use for capture.
MY_SIZE = 1 # number of samples we'll capture (simply a connectivity test)
TRUE = 1
...
#         if isinstance(vals[n], complex):
#             value = abs(vals[n])
#             final.append(value)
#         else:
#             final.append(vals[n])
#    print ["%f" % vals[n] for n in range(len(vals))]
    for n in range(len(DATA)):
        if DATA[n].imag >= 0:
            print ["%f + %fi" %(DATA[n].real,DATA[n].imag)]
        else:
            print ["%f - %fi" %(DATA[n].real,abs(DATA[n].imag))]
